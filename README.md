...
| `conformance-review` | Post-build best-practice review (criteria CR-1…CR-15) — run it once a build is TDD-green. |
...
| `conformance-reviewer` | After a build is TDD-green, review it against the best-practice criteria (CR-1…CR-15) — re-verifies tests via the real `iris_test` (not a self-graded `[SqlProc]`), reports findings + a scoped remediation plan, applies only unambiguous fixes after you confirm. |
