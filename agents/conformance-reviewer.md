...
`Skill(iris-interop-skills:conformance-review)` is the **single source of truth** (criteria CR-1…CR-15).
...
3. **Check every criterion** CR-1…CR-15 against the actual code. For each, cite `file:line` and state the
