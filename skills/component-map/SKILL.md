...
| Expose a REST endpoint (JSON in) | BS | `EnsLib.REST.Service` (is both `%CSP.REST`+`Ens.BusinessService`) | `EnsLib.HTTP.InboundAdapter` (own port) or adapterless via CSP web app | `XData UrlMap` `<Route>`; handler is an **instance** method → `..SendRequestSync(target,req,.resp)`; write to the **passed-in** `pOutput`. Web app `AutheEnabled=32` for Basic (`32`=password; `96` adds bit `64`=unauthenticated, which lets a privileged `UnknownUser` straight through). | `business-services`, `security` |
...
| Add an HL7 input to an **existing** router | Router (re-check!) | the router's engine must now be `EnsLib.HL7.MsgRouter.RoutingEngine` | — | a generic `EnsLib.MsgRouter.RoutingEngine` that was right for a RecordMap/JSON input becomes a **CR-6** violation the moment HL7 flows through it; it still compiles and still passes an end-to-end test. Split the router if it must serve both shapes, and re-run conformance | `bpl` |
| Route HL7 to multiple targets by type | Router | `Ens.Rule.Definition` (runtime `EnsLib.HL7.MsgRouter.RoutingEngine`) | — | `RuleAssistClass=EnsLib.HL7.MsgRouter.RuleAssist`; constrain on `docCategory`/`docName` (**not** an MSH compare — CR-5); if you must read MSH, name it: `Document.{MSH:MessageType.MessageCode}`, never `{MSH:9.1}`; `<send transform=… target=…/>`; item setting `BusinessRuleName` | `bpl` |
...
| Transform HL7→HL7 (same/diff version) | DTL | `Ens.DataTransformDTL` | — | `Create=Copy` (same ver) / `New` (cross-ver); address fields **by name**: `target.SetValueAt(v,"MSH:Security")`, not `"MSH:8"` | `transformations` |
...
| Read rows from an external/foreign DB | BO | `Ens.BusinessOperation` | `EnsLib.SQL.OutboundAdapter` | `..Adapter.ExecuteQuery(.rs,sql,p1)` → `rs.Next()`/`rs.Get("Col")`; **for anything typed (dates, decimals, NULLs) use `ExecuteQueryParmArray`/`ExecuteUpdateParmArray` with `$$$SqlVarchar`/`$$$SqlInteger` — needs `Include EnsSQLTypes`** (see `business-operations` §"Typed SQL parameters") | `business-operations` |
...
| Call an external REST endpoint (JSON out) | BO | `Ens.BusinessOperation` | **`EnsLib.HTTP.OutboundAdapter`** — there is no `EnsLib.REST.OutboundAdapter`; measured, it does not exist, and `EnsLib.REST.Operation` / `.GenericOperation` both declare the HTTP one | **`..Adapter.Post(.resp, formVarNames, data...)` takes NO URL** — measured: the URL is the adapter's `URL` setting. The URL-taking variant is `..Adapter.PostURL(url, .resp, formVarNames, data...)`, with the URL **first**. Writing `Post(.resp, url, body)` passes the URL as form-var names and posts to whatever the setting holds. `Credentials`,`SSLConfig`, finite `FailureTimeout`; `resp.StatusCode` is multidim → wrap in `Try{}` | `business-operations` |
...
| **Move a file A→B unchanged (relay / passthrough)** | BS **+** BO, **no custom class** | `EnsLib.File.PassthroughService` → `EnsLib.File.PassthroughOperation` | built-in file in/out | Body is **`Ens.StreamContainer`** (not an `Ens.Request`; `%Persistent` leftmost). `Filename` is a **Host** setting, `FilePath` an **Adapter** one; `%f` substitutes `OriginalFilename`. **A stream reads once** — a second `Read()` returns `""` silently, so rewind BEFORE reading, never after. No router: a relay decides nothing. Gated: `${CLAUDE_PLUGIN_ROOT}/BestPractices/examples/ch06_adapters/production-file-passthrough.cls` | `business-services`, `business-operations` |
...
| Map codes (M→1, planta→edificio, clinic names) | Lookup table + `Lookup()` | imported via Portal / API | — | DTL `..Lookup("GeneroSOAP",src.Genero,"")` — **two dots in a DTL**; the bare form compiles and then throws `<UNDEFINED>` at `Transform()` time. Bare `Lookup(...)` is the *business-rule* form. **Always pass the 3rd arg** (the on-miss default) — in a DTL only, since it is `Ens.Util.FunctionSet.Lookup`. From ObjectScript the method is `##class(Ens.Util.LookupTable).%GetValue(tbl, key, .exists)`, whose 3rd arg is a by-ref found-flag and **not** a default. | `lookup-tables`, `transformations` |
| Capture production errors to a sink | Alert router + sink BO | `EnsLib.MsgRouter.RoutingEngine` named **`Ens.Alert`** (its rule is an `Ens.Rule.Definition`) + `Ens.BusinessOperation` | `EnsLib.File.OutboundAdapter` | router **must** be named `Ens.Alert`; BO `MessageMap` on `Ens.AlertRequest`; per-item `AlertOnError=1` (portal label "Send Alert on Error") | `alerting` |
...
- `message-search-debug` + `Agent(subagent_type="iris-interop-skills:introspect-dont-guess")` — an agent, not a skill: typed-tool cheat-sheet and no-agent fallback (rule 2 above)
