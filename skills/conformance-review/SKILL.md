...
> file on disk), hard on **CR-15** (a hand BP whose `SendRequestAsync` leaves
> `pResponseRequired` at 1 with no `OnResponse` override) — both mechanical, no judgement
> involved — and once on "the conformance pass never ran". Answer it and stop again; the same
> unresolved condition is never raised twice. It speaks
