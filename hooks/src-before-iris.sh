#!/bin/sh
# PreToolUse source-of-truth gate (iris-interop-skills) — denies iris_doc(mode=put) when
# the class has no file on disk, so the namespace never becomes the only copy.
...
exec "$PY" "$DIR/src_before_iris.py"
