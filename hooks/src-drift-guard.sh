#!/bin/sh
# PostToolUse drift guard (iris-interop-skills) — warns when an iris_doc(mode=put) or an
# iris_production_item moved the NAMESPACE ahead of the tree. Advisory, never blocks.
...
exec "$PY" "$DIR/src_drift_guard.py"
